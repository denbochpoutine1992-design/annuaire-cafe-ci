...
export default function EditVendorPage() {
  ...
  if (loading) {
    return <p className="p-10 font-mono text-sm" style={{ color: "#8A7355" }}>Chargement...</p>;
  }
  ...
  return (
    <div className="min-h-screen">
      <nav className="px-6 md:px-12 py-4 border-b border-line flex items-center justify-between">
        ...
        <Link href={`/vendors/${id}`} className="font-mono text-xs uppercase tracking-wide" style={{ color: "#B85C38" }}>
            ...
            {photos.map((p) => (
              <div key={p.id} className="relative">
                ...
                <button
                  ...
                  style={{ background: "#B85C38", color: "#FBF5E9" }}
                >
                  ×
                </button>
              </div>
